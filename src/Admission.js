...
{ const [fees,setFees]=useState(0);
  ...
  function onNext()
  {
    ...
    promise
    .then(()=>{
      setSubmitFeesVisibleState('hidden block')
    })
    .then(()=>{
      let submitFees=document.querySelector(".submit-fees");
     
        submitFees.scrollIntoView(false);
      console.log(document.documentElement.scrollTop)
    })
     .then(()=>{
      setTimeout(setSubmitFeesVisibleState,1000,'visible')
      // setSubmitFeesVisibleState('visible')
    })
  ...
  function onNextEnter(event)
  {
   ...
   // promise
   //  .then(()=>{
   //    setSbmitFeesVisibleState('hidden block')
   //  })
   //   .then(()=>{
   //    let submitFees=document.querySelector(".submit-fees");
